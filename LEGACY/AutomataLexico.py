...
while i < len(codigofuente):
    ...
    if codigofuente[i].isalpha():
        ...
        if j < len(codigofuente) and codigofuente[j] == ".":
            ...
            while j < len(codigofuente) and codigofuente[j].isdigit():
                j += 1
            tokens.append(" " + codigofuente[i:j] + " <---> flotante <--->"+str(linea))
        else:
            tokens.append(" " + codigofuente[i:j] + " <---> entero <--->"+str(linea))
        col += j - i
        i = j
        continue
    # Identificar símbolos especiales
    if codigofuente[i] in simbolos_especiales:
        tokens.append(" " + codigofuente[i] + " <---> simbolo especial <--->"+str(linea))
        i += 1
        col += 1
        continue
    # Identificar operadores aritméticos y relacionales
    if codigofuente[i : i + 2] in operadores_relacionales:
        tokens.append(" " + codigofuente[i : i + 2] + " <---> operador relacional <--->"+str(linea))
        i += 2
        col += 2
        continue
    elif codigofuente[i] in operadores_relacionales:
        tokens.append(" " + codigofuente[i] + " <---> operador relacional <--->"+str(linea))
        i += 1
        continue
    if codigofuente[i : i + 2] in operadores_dobles:
        tokens.append(" " + codigofuente[i : i + 2] + " <---> operador aritmetico <--->"+str(linea))
        i += 2
        col += 1
        continue
    elif codigofuente[i] in operadores_aritmeticos:
        tokens.append(" " + codigofuente[i] + " <---> operador aritmetico <--->"+str(linea))
        i += 1
        col += 1
        continue
    else:
